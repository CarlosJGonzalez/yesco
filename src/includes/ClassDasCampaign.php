<?php
require ($_SERVER['DOCUMENT_ROOT']."/includes/DasApiSDK-2023/vendor/autoload.php");
use Das\CallRail;
 ...
 * CALLRAIL
 */
if( !defined( 'CALL_RAIL_ACCOUNT' ) ){
    define( 'CALL_RAIL_ACCOUNT', '748117466' );
}

if( !defined( 'CALL_RAIL_COMPANY_ID' ) ){
    define( 'CALL_RAIL_COMPANY_ID', 'COMe1f7493e22c740e2b5b2cb3fc5a8baf3' );
}
/********************************************************
 * ******************************************************/
...
class Das_Campaign
{
	...
	function __construct($db,$token,$client,$storeid = 0){
		...
		$this->client = $client;		   
		$this->company_id = CALL_RAIL_COMPANY_ID;
		$this->account = 	CALL_RAIL_ACCOUNT;
	...
	function addPortal($params){	
		if( !isset( $params['is_tollfree'] ) ){
			$params['is_tollfree'] = 0;
		}
		$data = array(
					"client" => $this->client_storeid,
					...
					"medium" => $this->db->escape($params['medium']),
					"channel" => $this->db->escape($params['channel']),
					"note" => $this->db->escape($params['notes']),
					"markup" => $this->db->escape($params['markup'])
				);
		...
		if(isset($params['createCTN'])){
			...
			if( !$ctn['is_error'] ){
				...
				return $this->db->insert("advtrack.campid_data",$data);
			}else{
				return $ctn;
			}
		}else{
