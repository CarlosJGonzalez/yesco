...
$locations = $db->rawQuery ("SELECT storeid, companyname, yext_logo FROM locationlist WHERE yext_logo IS NULL");
...
if ($db->count > 0){
    ...
    foreach ($locations as $location) {
		...
		if($yext_location_info['id']){
			if(isset($yext_location_info["logo"])){
				$update_data["yext_logo"] =  $yext_location_info["logo"]["url"];
			}else{
				$location_data["logo"] = ["url" => LOCAL_CLIENT_URL."img/gallery/yext/logo.jpg", "description"=>CLIENT_NAME." LOGO"];
				$update_data["yext_logo"] = LOCAL_CLIENT_URL."img/gallery/yext/logo.jpg";
			}
		}else{
			$update_data["yext_logo"] = LOCAL_CLIENT_URL."img/gallery/yext/logo.jpg";
		}
