...
$locations = $db->rawQuery ("SELECT storeid, companyname, yext_photos FROM locationlist WHERE `yext_photos` IS NULL");
...
if ($db->count > 0){
    ...
    foreach ($locations as $location) {
		...
		$yext_location_info = $yext_location[0];
		
		//If the location already exists on Yext, the data will be downloaded from it and updated in the locationlist table
		if($yext_location_info['id']){
			if(isset($yext_location_info["photos"])){
				$yext_photos = []; 

				foreach ( $yext_location_info["photos"] as  $value) {
					$yext_photos[] =  $value["url"] ;
				}
				
				$update_data["yext_photos"] = json_encode($yext_photos);
			}else{
				$location_data["photos"] = [["url" => LOCAL_CLIENT_URL."img/gallery/yext/yext_1.jpg", "description"=>CLIENT_NAME." Picture1"],
											["url" => LOCAL_CLIENT_URL."img/gallery/yext/yext_2.jpg", "description"=>CLIENT_NAME." Picture2"],
											["url" => LOCAL_CLIENT_URL."img/gallery/yext/yext_3.jpg", "description"=>CLIENT_NAME." Picture3"],
											["url" => LOCAL_CLIENT_URL."img/gallery/yext/yext_4.jpg", "description"=>CLIENT_NAME." Picture4"],
											];
							
				$update_data["yext_photos"] = json_encode([LOCAL_CLIENT_URL."img/gallery/yext/yext_1.jpg",
														   LOCAL_CLIENT_URL."img/gallery/yext/yext_2.jpg",
														   LOCAL_CLIENT_URL."img/gallery/yext/yext_3.jpg",
														   LOCAL_CLIENT_URL."img/gallery/yext/yext_4.jpg",
														  ]);
			}	
		}else{
			$update_data["yext_photos"] = json_encode([LOCAL_CLIENT_URL."img/gallery/yext/yext_1.jpg",
