...
if( count($_POST) ){
	...
	$itotal = $reviews['info']['total_items'];
	$itotal1 = $reviews['info']['total_items'];

	foreach ($reviews['data'] as $review) {

		$reviw_id = base64_encode($_SESSION['client'].$review['review_id'] );
		$dasClient = new Das_Client($db,$token_api,$_SESSION['client'],$review['storeid']);
		...
		switch ($review['portal']) {
			case 'Google':
				...
				$reply ='<a class="respondModal" href="#" data-toggle="modal" data-target="#respondModal" data-id="'.$reviw_id.'" data-portal="'.$review['portal'].'"><i class="mx-1 text-blue fas fa-bullhorn"></i></a>';
				break;
			case 'Facebook':
			...
			case 'Yelp':
				...
				$reply ='<a href="https://biz.yelp.com/login" target="_blank" ><i class="mx-1 text-blue fas fa-bullhorn"></i></a>';
				break;												
			default:
				$icon = '';
				$view = '';
		...
		$action .= isset($reply) ? $reply: '';

		$data[] = array(
								"date"  	  => $review['create_date'],
								"storeid"  	  => isset( $clientInfo['data'][0]['name'] ) ? $clientInfo['data'][0]['name'] : $review['storeid'],
								"source"  	  => $icon,
								"rating"  	  => $rating,
								"review"  	  => $comment,							
						    );
