			...
			<main role="main" class="col-md-9 ml-sm-auto col-lg-10 p-0">
				...
				<div class="py-3 px-4">
					...
					if (isset($_GET["region"])){
						...
						$region_storeids = getLocationByRegion($db,$region);
						...
						<table class="table" id="campaign_details">						
							...
							<tbody>
								<tr>			
									...
									$storeids = $clientObj->getLocations( $client );
									$client_corp = $clientObj->getClient( $client );

									if(!$storeids['is_error'] && $storeids['info']['count'] > 0){						
										$storeids  = $storeids['data'];
									}else{
										$storeids = [];	
									}

									$storeids[] = $client_corp['data'][0];
									...
									foreach ($storeids as $storeid) {
										...
										$campidInfo = $campidObj->getCampId($campid,$_SESSION['client'],( $storeid['storeid_api'] == '0' ) ? null : $storeid['storeid_api'] );
										if($campidInfo['is_error'] || $campidInfo['count'] <= 0){
											continue;
										}								

										$params  = array(
											'storeid' => $storeid['storeid_api'], 
											'gte' => (string)$from, 
											'lte' => (string)$to,
											'campid' => $campid,
										);

										$debitInfo= $campidObj->getDebits($client,$storeid['storeid_api'],$params);
										...
										$reportCampiagn = $report->getCampaignReport($_SESSION['client'],$params);
										...
										if( ($clicks + $imps + $cost + $tot_leads) <= 0){
											continue;
										}
										...
										$client_name = 'N/A';
										if(isset($storeid['name']) && $storeid['name'] != ''){
											$client_name = $storeid['name'];
										}
										...
										if($compare){
											...
											$leadsPrev = $report->getLeadReport($client,$params_prev);
											...
											$tot_leads_prev = 0;
											$lead_desc_prev = "";
											if(!$leadsPrev['is_error'] && $leadsPrev['info']['count'] > 0){
												$leads_info = getLeadInfo($leadsPrev['data']);
										...
										<th scope="row"><?php echo $storeid['storeid_api'];?></th>						
