					   ...
					   <tfoot>
						...
						$flag_security=true;
							//if($_SESSION['storeid']>  0){
								...
								$campids = $db->rawQuery("select * from advtrack.campid_data where client like '9018%' and active='Y'");

								if(count($campids)){	
									foreach($campids as $campid1){
										$campid = $campid1['campid'];
									//echo $campid .'<br/>';
										$campaign = getCampaignDetails($campid1['client'],$campid,date("Y-m-d",strtotime($from)),date("Y-m-d",strtotime($to)));
										if(!$campaign->message) {
											$campaigns = $campaign->records;
										//echo '<pre>'.print_r(json_encode($campaigns)).'</pre>';
										//$db->setTrace (true);
											$clients = getClient($campid1['client']);
											$client_id = $clients->client;
											$client_name = $clients->name;

											$campids = getCampid($campid1['client'], $campid);
											$campid_name = $campids->name;

											$leads = getLeads($campid1['client'],$campid,date("Y-m-d",strtotime($from)),date("Y-m-d",strtotime($to)));
											$leads = $leads->records;
										//print_r ($db->trace);
