...
if(count($_POST) and isset($_SESSION['user_id'])){
	...
	$db = new MysqliDb ('localhost', 'root', 'dasflorida', 'linkedin');
	$master_tb = 'page_share_stats';
	...
	$cols_db = array (
					//'date',
					'storeid',
					'FORMAT(SUM(impression), 0) as impression',
					'FORMAT(SUM(click), 0) as click',
					'FORMAT(SUM(engagement), 0) as engagement',
					'FORMAT(SUM(likes), 0) as likes',
					'FORMAT(SUM(shareCount), 0) as shareCount',
	...
	if($_SESSION['storeid'] <= 0){
		...
		$logs = $db->where('client',$_SESSION['client'].'%','LIKE')->groupBy('client,storeid')->get($master_tb, Array ($start, $rowperpage), $cols1);

		//This query is only used to set  iTotalRecords and iTotalDisplayRecords
		$logsNoLimit = $db->where('client',$_SESSION['client'].'%','LIKE')->groupBy('client,storeid')->get($master_tb, null, $cols1);
			...
			if(isset($log[$col])){
				...
				if($col == 'storeid'){
					if ($log[$col] == 0) {
						...
						$info['companyname'] = $companyname['companyname'];
					}					
