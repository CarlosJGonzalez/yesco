...
if(count($_POST) and isset($_SESSION['user_id'])){
	...
	if($_SESSION['storeid'] <= 0){
		$cols1 = array (
					...
					"ROUND( ( SUM(spend) * 1.25 ), 2) as spend",
					//"ROUND( SUM(spend) , 2) as spend",
				);

		$logs = $db->where('client',$_SESSION['client'].'%','LIKE')->groupBy('client,campaignname')->get($master_tb, Array ($start, $rowperpage), $cols1);
		...
		$logs = $db->groupBy('client,campaignname')->get($master_tb, Array ($start, $rowperpage), $cols1);
	}
...
function filterBySearch(&$db, $search_value){
	$db->where ("(client like ? or date like ? or campaignname like ?)", Array("%$search_value%", "%$search_value%", "%$search_value%"));
}
