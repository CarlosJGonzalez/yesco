...
<html lang="en">
  ...
  <body class="bg-light cbp-spmenu-push">
    ...
    <div class="container-fluid">
      ...
      <div class="row">
		...
		<?php 
			...
			$locationList = $db->where("storeid",$_GET['storeid'])->getOne("locationlist");
			//$state_name = strtolower(getStateName($locationList['state']));
			
			//Get Yelp location information
			$db->Where("client = ? AND storeid = ?",array($_SESSION['client'], $_GET['storeid']));
			$yelp_location = $db->getOne("advtrack.yelp_locations", 'yelp_bizid');
			
			//Get Yelp camp_id from the campid_data table
			$db->Where("client = ? AND name = ? AND active = ?",array($_SESSION['client'].'-'.$_GET['storeid'], 'Yelp', 'Y'));
			$yelp_campaign_data = $db->getOne("advtrack.campid_data", 'campid');

			$locationList['twitter'] = ($locationList['twitter']) ? $locationList['twitter'] : 'FullyPromoted';
			$locationList['facebook']= ($locationList['facebook']) ? $locationList['facebook'] : 'facebook.com/fullypromoted.corporate/';
			$locationList['instagram']= ($locationList['instagram']) ? $locationList['instagram'] : 'fullypromoted';
			$locationList['linkedin']= ($locationList['linkedin']) ? $locationList['linkedin'] : 'https://www.linkedin.com/company/fully-promoted/';
			$locationList['pinterest']= ($locationList['pinterest']) ? $locationList['pinterest'] : 'https://www.pinterest.ca/embroidme/';
			$locationList['youtube']= ($locationList['youtube']) ? $locationList['youtube'] : 'https://www.youtube.com/c/embroidme';
			
			$socialMediaAccess = $db->where("client", $_SESSION['client'], "=" )
									->where("storeid", $_GET['storeid'], "=" )
									->getOne("das_contract.social_media_access");
		?>
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 p-0 mb-4">
						...
						<form action="" method="post" id="frm_core_information">
							...
							<div class="row">
								<div class="col-sm-4 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Business Name <span data-toggle="tooltip" data-placement="top" title="Enter the business’s full name without “of” or “-” (i.e., Fully Promoted City, ST)."><i class="far fa-question-circle"></i></span></label>
									<input type="text" class="form-control rounded-bottom rounded-right" name="companyname" value="<?=$locationList['companyname']?>" required>
								</div>
								 <div class="col-sm-4 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Display Name <span data-toggle="tooltip" data-placement="top" title="Enter the business’s name as it should appear in local listings, etc. without “of” or “-” (i.e., Fully Promoted City)."><i class="far fa-question-circle"></i></span></label>
									<input type="text" class="form-control rounded-bottom rounded-right" name="displayname" value="<?=$locationList['displayname']?>" required>
								</div>
								<div class="col-sm-4 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Store ID</label>
									<input type="text" class="form-control rounded-bottom rounded-right" name="storeid" value="<?=$locationList['storeid']?>" readonly>
								</div>
								...
								<?php if(roleHasPermission('edit_location_rep', $_SESSION['role_permissions'])){ ?>
								<div class="col-sm-6 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Rep</label>
									<?
									$sql_reps = "SELECT * FROM reps";
									$reps = $db->rawQuery($sql_reps);
									...
									<div class="select-wrapper">
										<select name="rep" class="form-control bg-white border rounded">
											...
											foreach($reps as $rep){
											?>
												<option value="<?=$rep['id']?>" <? if($rep['id'] == $locationList['rep']) echo "selected";?>><?=$rep['name']?></option>
											<?
											}
											?>
										</select>
									...
									<? 
										...
										echo '<input type="text" class="form-control rounded-bottom rounded-right" name="not_rep" value="There are not representatives yet." readonly>';
									}
									?>
								</div>
								<?php } ?>
							...
							<div class="row">
								...
								<div class="col-sm-12">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Address <span data-toggle="tooltip" data-placement="top" title="Enter your business’s street address without your suite or unit information."><i class="far fa-question-circle"></i></span></label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="address" value="<?php echo $locationList['address']?>" required>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Address 2 <span data-toggle="tooltip" data-placement="top" title="Enter your unit # or suite # (i.e., Suite 105 or Unit B)"><i class="far fa-question-circle"></i></span></label>
								...
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">City <span data-toggle="tooltip" data-placement="top" title="Enter the city that would be recognized by USPS for your address."><i class="far fa-question-circle"></i></span></label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="city" value="<?php echo $locationList['city']?>" required>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">State <span data-toggle="tooltip" data-placement="top" title="Enter the 2-letter abbreviation for your state."><i class="far fa-question-circle"></i></span></label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="state" value="<?php echo $locationList['state']?>" required>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Zip <span data-toggle="tooltip" data-placement="top" title="Enter the 6-digit zip code for your business address."><i class="far fa-question-circle"></i></span></label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="zip" value="<?php echo $locationList['zip']?>" required>
									</div>
								</div>
								<div class="col-12">
				...
				<div class="row mb-4">
					<div class="col-12">
						...
						<form action="" method="post"  id="frm_business_details">
							...
							<!-- Business Hours -->
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Hours of Operation <span data-toggle="tooltip" data-placement="top" title="Select your hours of operation for each day. If closed on certain days, mark that field. If you are only available by appointment on certain days, mark that field. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
							...
							<div class="field form-group ">
								...
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Holiday Hours <span data-toggle="tooltip" data-placement="top" title="Add your holiday hours by selecting dates from the calendar and adding your hours or marking your business as closed, etc. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
								<div class="border rounded-bottom rounded-right p-2">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">First Name <span data-toggle="tooltip" data-placement="top" title="Enter your first name."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Last Name <span data-toggle="tooltip" data-placement="top" title="Enter your last name."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Phone <span data-toggle="tooltip" data-placement="top" title="Enter the best phone number for contacting you."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Email Address <span data-toggle="tooltip" data-placement="top" title="Enter your work email address. This is used for sending out email blasts with important information regarding the brand as well as for emailing you directly for issues related to your business."><i class="far fa-question-circle"></i></span></label>
			...
			<div class="card-deck mb-4 dashboard">
				<div class="card rounded-0 border-0 box p-3">
					...
					<div class="card-body p-0">
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Primary Location Email <span data-toggle="tooltip" data-placement="top" title="Enter the best email address(es) for your location."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Website <span data-toggle="tooltip" data-placement="top" title="Enter the location part of your local website’s URL in lowercase letters separating each word with a dash and using no spaces. It must contain the 2-letter abbreviation of your location's state at the end of the URL. (i.e., austin-tx or boynton-beach-fl)."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Alternative Website <span data-toggle="tooltip" data-placement="top" title="If you have another website for your business, enter the URL here (i.e., your shop site)."><i class="far fa-question-circle"></i></span></label>
							<input type="url" class="form-control web_info" name="alternative_website_url" value="<?php echo $locationList['alternative_website_url']?>" required>
						</div>
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Business Phone <span data-toggle="tooltip" data-placement="top" title="Enter your business’s complete phone number (i.e., xxx-xxx-xxxx)."><i class="far fa-question-circle"></i></span></label>
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Send Contact Form To <span data-toggle="tooltip" data-placement="top" title="Enter the email address that contact forms from the website should be sent to."><i class="far fa-question-circle"></i></span></label>
				...
				<div class="card rounded-0 border-0 box p-3">
					...
					<div class="card-body p-0">
						...
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Yelp Id</label>
							<input type="text" class="form-control rounded-bottom rounded-right social_med" name="yelp_bizid" value="<?php echo $yelp_location['yelp_bizid']?>" <?php if (!isset($yelp_campaign_data['campid'])) echo 'disabled'; ?>>
						</div>
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Yelp Campaign Id</label>
							<input type="text" class="form-control rounded-bottom rounded-right social_med" name="yelp_campid" value="<?php echo $yelp_campaign_data['campid']?>" readonly>
						</div>
						...
						<?php } ?>
						<?php if(roleHasPermission('edit_social_media_access', $_SESSION['role_permissions'])){ ?>
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">GMB Access</label>
								<div class="select-wrapper">
									<select name="gmb_access" class="form-control bg-white border rounded social_med">
										<option value="">--</option>
										<option value="none" <? if($socialMediaAccess['gmb_access'] == 'none') echo "selected";?>>None</option>
										<option value="owner" <? if($socialMediaAccess['gmb_access'] == 'owner') echo "selected";?>>Owner</option>
										<option value="manager" <? if($socialMediaAccess['gmb_access'] == 'manager') echo "selected";?>>Manager</option>
									</select>
								</div>
							</div>
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">FB Access</label>
								<div class="select-wrapper">
									<select name="fb_access" class="form-control bg-white border rounded social_med">
										<option value="">--</option>
										<option value="none" <? if($socialMediaAccess['fb_access'] == 'none') echo "selected";?>>None</option>
										<option value="admin" <? if($socialMediaAccess['fb_access'] == 'admin') echo "selected";?>>Admin</option>
										<option value="editor" <? if($socialMediaAccess['fb_access'] == 'editor') echo "selected";?>>Editor</option>
									</select>
								</div>
							</div>
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">LI Access</label>
								<div class="select-wrapper">
									<select name="li_access" class="form-control bg-white border rounded social_med">
										<option value="">--</option>
										<option value="yes" <? if($socialMediaAccess['li_access'] == 'yes') echo "selected";?>>Yes</option>
										<option value="no" <? if($socialMediaAccess['li_access'] == 'no') echo "selected";?>>No</option>
									</select>
								</div>
							</div>
						<?php } ?>
			...
			<div class="row mb-4">
				<div class="col-12">
					<div class="box p-3">
						...
						<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Business Logo <span data-toggle="tooltip" data-placement="top" title="Your business logo as it should appear in your local listings."><i class="far fa-question-circle"></i></span></label>
							<div class="border rounded-bottom rounded-right p-2">
				...
				<div class="col-12">
					<div class="box p-3">
						<form action="" method="POST" id="frm_additional_attributes">
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Year Established <span data-toggle="tooltip" data-placement="top" title="Enter the year the brand was created. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
							...
							<div class="field form-group">
								<input type="hidden" name="change_products" value="0"/>
								<?php $products = $locationList['yext_products']!= ""?explode(';' , $locationList['yext_products']):[]; ?>
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Products <span data-toggle="tooltip" data-placement="top" title="Enter the products that your business offers. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
								<div id="prod-container" class="border rounded-bottom rounded-right p-2">
									<div class="row">
										<div class="form-inline col-sm-3 mb-2">
											...
											<input type="text"  class ="form-control rounded productChanged apiLimit" name="products[]" value="<?=(count($products)>0)?trim(array_shift($products)):'';?>"/>   
										</div>
										...
										<div class="form-inline col-sm-3 mb-2">
											<i class="far fa-trash-alt removeProdRow cursor-pointer mr-2" aria-hidden="true"></i> 
											<input type="text" class="form-control rounded apiLimit" name="products[]" value="<?=trim($prod);?>"/>   
										</div>
										<?php } ?>
									</div>
								</div>
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Services <span data-toggle="tooltip" data-placement="top" title="Enter the services that your business offers. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
								...
								<div id="serv-container" class="border rounded-bottom rounded-right p-2">
									<div class="row">
										<div class="form-inline col-sm-3 mb-2">
											...
											<input type="text"  class ="form-control rounded apiLimit" name="services[]" value="<?=(count($services)>0)?trim(array_shift($services)):'';?>"/>   
										</div>
										...
										<div class="form-inline col-sm-3 mb-2">
											<i class="far fa-trash-alt removeServRow cursor-pointer mr-2" aria-hidden="true"></i> 
											<input type="text" class="form-control rounded apiLimit" name="services[]" value="<?=trim($serv);?>"/>   
										</div>
										<?php } ?>
									</div>
								</div>
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Specialties <span data-toggle="tooltip" data-placement="top" title="If your business has any specialties, enter them here. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
								...
								<div id="specialties-container" class="border rounded-bottom rounded-right p-2">
									<div class="row">
										<div class="form-inline col-sm-3 mb-2">
											...
											<input type="text"  class ="form-control rounded w-90 apiLimit" name="specialties[]" value="<?=(count($specialties)>0)?array_shift($specialties):'';?>"/>   
										</div>
										...
										<div class="form-inline col-sm-3 mb-2">
											...
											<input type="text" class="form-control rounded w-90 apiLimit" name="specialties[]" value="<?=$spec;?>"/>   
										</div>
										<?php } ?>
									</div>
								</div>
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Certifications &amp; Affiliations <span data-toggle="tooltip" data-placement="top" title="Enter any relevant certifications or affiliations. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Brands <span data-toggle="tooltip" data-placement="top" title="Enter the brand names for products that your business offers. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
								...
								<div id="brands-container" class="border rounded-bottom rounded-right p-2">
									<div class="row">
										<div class="form-inline col-sm-3">
											...
											<input type="text" class="form-control rounded w-90 apiLimit"  name="brands[]" value="<?=(count($brands)>0)?array_shift($brands):'';?>"/>   
										</div>
										<?php for($i=0;$i<count($brands);$i++){
											...
											<div class="form-inline col-sm-4">
												<i class="far fa-trash-alt removeBrandRow cursor-pointer mr-2" aria-hidden="true"></i> 
												<input type="text" class="form-control rounded w-90 apiLimit" name="brands[]" value="<?=$brand;?>"/>   
											</div>
										<?php } ?>
									</div>
								</div>
							...
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Tagline <span data-toggle="tooltip" data-placement="top" title="Enter your business’s tagline. This will appear in your local listings."><i class="far fa-question-circle"></i></span></label>
			...
			<div class="row mb-4">
				<div class="col-12">
					<div class="box p-3">
						<form action="" method="POST" id="frm_additional_information">
						<h2 class="text-uppercase h4 text-dark d-flex flex-wrap">Additional&nbsp;<span class="text-blue">Information</span></h2>
							<div class="form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Metro Area</label>
								<div class="select-wrapper">
									<select name="metro" id="metro" class="form-control bg-white border rounded" required>
										<option value="">--</option>
										<?php 
										$metros = $db->rawQuery("SELECT * FROM option_values WHERE `option` = 'metro_area' AND `value` IS NOT NULL AND `display_name` IS NOT NULL ORDER BY display_name ASC");
										foreach($metros as $m){
										?>
										<option value="<?php echo $m['display_name']; ?>" <?php if($locationList['metro'] == $m['display_name']) echo "selected"; ?>><?php echo $m['display_name']; ?></option>
										<?php } ?>
									</select>
									<small>+</small> <a href="#" class="addMetro"><small><b>Add New Metro</b></small></a>
									</br>
									<small><b>Note:</b> This affects the metro area on the website.</small>
								</div>
							</div>
							<div class="field form-group" id="newMetroContainer">    
							</div>
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Comments/Notes</label>
								<textarea class="form-control custom-control" name="comments" rows="3"><?=$locationList['comments']?></textarea>     
							</div>
							<div class="field form-group">
								<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">FB Interests</label>
								<textarea class="form-control custom-control" name="interests" id="interests" rows="3"><?=$locationList['interests']?></textarea>     
							</div>
							<div class="text-center">
								<input type="submit" class="btn save bg-dark-blue text-white btn-sm px-4" value="Save" id="additional_information" />
								<input type="hidden" name="storeid" value="<?php echo $_GET['storeid']; ?>"/>
							</div>
						</form>
					</div>
	...
	<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDcZQgytDL7Lwlc76bzc7MA5bt-cq2mqO8&callback=initMap"></script>
    <script>	
	var forms_action = location.protocol+'//'+location.hostname+'/yextAPI/localsites2yext.php';
		...
		$(document).on('change', '#metro', function(e){
			if($("#newMetroInput").length == 1){
				$('#newMetroInput').remove();
			}
		});
		
		$(".addMetro").click(function(e){
			e.preventDefault();
			let metroLabel = '<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">New Metro</label>';
			let metroInput = '<i class="far fa-trash-alt removeMetroRow cursor-pointer mr-2" aria-hidden="true"></i><input type="text" name="new_metro" id="new_metro" class="form-control custom-control">';
			let note = '<small><b>Note:</b> This affects the metro area on the website.</small>';				
			let metroInputExist = (typeof $("#newMetroInput") !== 'object') ? true : false;
			
			//Reset dropdown metro list to default when adding a new metro
			$("#metro option:eq(0)").prop("selected", true);

			if($("#newMetroInput").length == 0){
				$("#newMetroContainer").append('<div id="newMetroInput">'+metroLabel+metroInput+note+'</div>');
			}
		});
		
		$(document).on('click', '.removeMetroRow', function(e){
			$('#newMetroInput').remove();
		});
