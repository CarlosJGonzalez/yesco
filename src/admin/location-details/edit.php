...
<html lang="en">
  <head>
	...
	<script>
		function initMap() {
			var lat_exist = document.querySelector( "#lat" ) !== null;
			var lng_exist = document.querySelector( "#lng" ) !== null;

			if( lat_exist && lng_exist ){
				var lat = document.querySelector( "#lat" );
				var lng = document.querySelector( "#lng" );

				if ( lat.value != '' && lat.value != '0.000000' && lng.value != '' && lng.value != '0.000000' ) {
					...
					console.log( 'No Lat nor Lon found.' );
					/**
					 * pull Lat and Lng from Google API
					 */
					...
					var storeid = document.querySelector( "input[name='storeid']" );
					...
					form.append( 'storeid', storeid.value );

					async function pullLatLng( url = "", data = {} ){
						await fetch( url, {
							method: "POST",
							mode: "no-cors",
							cache: "no-cache",
						...
						.then( async (response) => {
							...
							var message = document.querySelector( "div.alert.alert-danger" );
							var error = await response.json();
							message.innerHTML = error.message;
							message.style.display = 'block';
						})
						.then( data => {
							if( data && data.hasOwnProperty( 'updated' ) ){
								var lat = document.querySelector( "#lat" );
								var lng = document.querySelector( "#lng" );
								lng.value = data.lng;
								lat.value = data.lat;
					...
					pullLatLng( "https://localyesco.com/admin/location-details/xt_google_lat_lng.php", form );
				}
			}
		}	
	</script>	
    ...
    <div class="container-fluid">
      ...
      <div class="row">
		...
		<?php 
			...
			$locationList['facebook']= ($locationList['facebook']) ? $locationList['facebook'] : '';
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 p-0 mb-4">
						...
						<form action="" method="post" id="frm_core_information">
							...
							<div class="row">
								<div class="col-sm-4 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Business Name <span data-toggle="tooltip" data-placement="top" title="Enter the business’s full name without “of” or “-” (i.e., Yesco City, ST)."><i class="far fa-question-circle"></i></span></label>
									<input type="text" class="form-control rounded-bottom rounded-right" name="companyname" value="<?=$locationList['companyname']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?> required>
									<small><b>Note: </b>Enter the City, ST in this field. For example: Vero Beach, FL</small>
								</div>
								 <div class="col-sm-4 form-group">
									<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Display Name <span data-toggle="tooltip" data-placement="top" title="Enter the business’s name as it should appear in local listings, etc. without “of” or “-” (i.e., Yesco City)."><i class="far fa-question-circle"></i></span></label>
									<input type="text" class="form-control rounded-bottom rounded-right" name="displayname" value="<?=$locationList['displayname']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?> required>
									<small><b>Note: </b>Enter the location's full name in this field. For example: Yesco Vero Beach, FL</small>
								</div>
								<div class="col-sm-4 form-group">
								...
								<div class="col-sm-6 form-group">
									...
									<input type="text" class="form-control rounded-bottom rounded-right" name="yext_featured_message_url" value="<?php echo $locationList['yext_featured_message_url']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
								</div>
							...
							<div class="row">
								...
								<div class="col-sm-12">
									<div class="field form-group">
										...
										<input type="text" class="form-control rounded-bottom rounded-right address" name="address" value="<?php echo $locationList['address']?>" required <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
										<small><b>Note: </b>Please do not include the Suite #, Unit #, etc. in this field. This belongs in Address 2.</small>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Address 2</label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="address2" value="<?php echo $locationList['address2']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
										<small><b>Note: </b>Enter the Suite #, Unit #, etc. as needed according to the postal service.</small>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">City</label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="city" value="<?php echo $locationList['city']?>" required <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">State </label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="state" value="<?php echo $locationList['state']?>" required <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
									</div>
								</div>
								<div class="col-sm-6">
									<div class="field form-group">
										<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Zip </label>
										<input type="text" class="form-control rounded-bottom rounded-right address" name="zip" value="<?php echo $locationList['zip']?>" required <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
									</div>
								</div>
								<div class="col-12">
			...
			<div class="card-deck mb-4 dashboard">
					...
					<div class="card-body p-0">
						...
						<div class="field form-group">
							...
							<div class="input-group mb-3">
							  ...
							  <input type="text" class="form-control rounded-bottom rounded-right web_info" name="url" id="basic-url" aria-describedby="basic-addon3" value="<?php echo $locationList['url']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
							</div>
						</div>
						<div class="field form-group">
						...
						<div class="field form-group">
							...
							<input type="text" class="form-control rounded-bottom rounded-right phoneInput web_info phone" name="phone" value="<?php echo $locationList['phone']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
						</div>
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">GA4 Property Id</label>
							<input type="text" class="form-control rounded-bottom rounded-right web_info" name="ga4_property_id" value="<?php echo $locationList['ga4_property_id'] ?? '';?>">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Measurement Id</label>
							<input type="text" class="form-control rounded-bottom rounded-right web_info" name="measurement_id" value="<?php echo $locationList['measurement_id']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
						</div>						
						<div class="form-group">
				...
				<div class="card rounded-0 border-0 box p-3">
					...
					<div class="card-body p-0">
						<!--<div class="field form-group">
							<label class="font-12 text-uppercase bg-light py-1 px-2 mb-0 rounded-top letter-1 text-dark">Local Hashtag</label>
							<div class="input-group mb-1">
							  <div class="input-group-prepend">
								<span class="input-group-text rounded-left" id="basic-addon3"><?php  echo '#yesco'; ?></span> 
							  </div>
							<input type="text" class="form-control rounded-bottom rounded-right social_med" name="local_hashtag" value="<?php echo $locationList['local_hashtag']?>">
							</div>
						</div>--->
						<div class="field form-group">
							...
							<div class="field form-group">
								...
								<div class="select-wrapper">
									<select name="gmb_access" class="form-control bg-white border rounded social_med">
										...
										<option value="optout" <? if($socialMediaAccess['gmb_access'] == 'optout') echo "selected";?>>Opt Out</option>
									</select>
								</div>
							</div>
							<div class="field form-group">
								...
								<div class="select-wrapper">
									<select name="fb_access" class="form-control bg-white border rounded social_med">
										...
										<option value="editor" <? if($socialMediaAccess['fb_access'] == 'optout') echo "selected";?>>Opt Out</option>
									</select>
								</div>
							</div>
							<div class="field form-group">
								...
								<div class="select-wrapper">
									<select name="li_access" class="form-control bg-white border rounded social_med">
										...
										<option value="no" <? if($socialMediaAccess['li_access'] == 'optout') echo "selected";?>>Opt Out</option>
									</select>
								</div>
							</div>
						<?php } ?>
										...
										foreach($photo as $index => $value){?>
											<div class="img-thumbnail position-relative d-inline-block mb-2">
												<div class="p-1 bg-white position-absolute top-right">
													<i data-id = <?=$index;?> class="far fa-trash-alt removephoto cursor-pointer" title="Delete" aria-hidden="true"></i>
												</div>
												<img src="<?php echo $value?>" class="max-200" />								
											</div>
										<?php } 
										}?>
									</div>
					...
					<div class="box p-3">
						<form action="" method="POST" id="frm_additional_attributes">
							...
							<div class="field form-group">
								...
								<input type="text" class="form-control rounded-bottom rounded-right" name="yearestablished" value="<?php echo $locationList['yearestablished']?>" <?php if(!roleHasPermission('update_specific_fields', $_SESSION['role_permissions'])) echo 'readonly' ?>>
							</div>
							<div class="field form-group">
	...
	var forms_action = location.protocol+'//'+location.hostname+'/yextAPI/localsites2yext.php';
    	$(document).ready(function(){
			...
			$(document).on('click', '.removephoto', function(e){
				if( ! confirm( "Would you like delete this image?") ){
					return;
				}
				let id = parseInt( e.currentTarget.dataset.id );
				let storeid = document.querySelector( "input[name='storeid']" ).value;
				var frmrp = new FormData();
				frmrp.append( 'photo_index', id );
				frmrp.append( 'store_id', storeid );

				async function remove_photo_gallery( url, data ){
					await fetch( url, {
						method: "POST",
						mode: "no-cors",
						cache: "no-cache",
				...
				remove_photo_gallery( 'xt_remove_photo_gallery.php', frmrp )
				.then( response => {
