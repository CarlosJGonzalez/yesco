...
foreach($locations as $location){
	...
	//$state = strtolower(getStateName($current_data[0]["state"]));
	//Get state name
	$cols = Array ("state_abbr", "state_name");
	$db->where("state_abbr", $current_data[0]["state"], "=");
	$result = $db->getOne("state_names", $cols);
	$state = (isset($result['state_name'])) ? strtolower($result['state_name']) : strtolower($current_data[0]["city"]);
	...
	if(array_key_exists("pinterest", $values) && ($values["pinterest"] != $current_data[0]["pinterest"])){
		$hasupdates["Pinterest"] =  $update_data["pinterest"] = trim($values["pinterest"], "/");			
	}
