...
<?php
	...
	if(isset($_POST["type"]) && $_POST["type"] == 'store'){
		$posts = $db->get('social_media__local_posts a',null,"concat(DATE_FORMAT(a.date,'%W'),',',DATE_FORMAT(a.date,'%b %d %Y')) as date,a.id as id,post,link,image, a.date as curdate, portal, notes,video,boost");
	}else{
		$posts = $db->get('social_media_posts a',null,"concat(DATE_FORMAT(a.date,'%W'),',',DATE_FORMAT(a.date,'%b %d %Y')) as date,a.id as id,post,link,image, a.date as curdate, portal, '' as notes,video,'' as boost");
	}
...
<!--				<input type="checkbox" name="posts" class="mr-2" />-->
				<div class="border flex-grow rounded box-shadow">
					...
					<div class="p-2 position-relative">
						...
						<div class="dropup position-absolute p-bottom-right cursor-pointer">
							<div class=" rounded-bottom-right rounded-bottom-right text-secondary bg-light py-1 px-2 border" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="optionsButton">Options</div>
		...
		<?php }
		}else echo "Sorry there are not posts. Please, change your search and try again.,"; ?>
</div>