...
<html lang="en">
  ...
  <body class="bg-light cbp-spmenu-push">
        	...
        	<div class="px-4 py-3">
				...
				<div class="row">
					...
					<div class="col-md-8 col-lg-9 col-xl-10 order-md-first">
						...
						$campaigns = $db->get("mailchimp_campaigns");
						if($db->count>0){
								...
								<div class="border rounded p-2 mb-3">
									<div class="row align-items-center">
										<div class="d-none d-md-block col-md-2 text-center">
											<?php 
											if($template['is_error'] == 0 || !empty($template['thumbnail']))
												echo '<img src="'.$template['thumbnail'].'" class="img-fluid max-h-100">';
											else
												echo '<img src="https://via.placeholder.com/250x400" class="img-fluid max-h-100">';
											?>
										</div>
										<div class="col-sm-6">
											<span class="h3 mb-1 text-capitalize span-title-editable"><?php echo $campaign_details['settings']['title']; ?></span>
											<input type='text' class='form-control rounded-bottom rounded-right input-title-editable' name='title-editable' value="<?php echo $campaign_details['settings']['title']; ?>" data-id="<?php echo $campaign['campaign_id']; ?>" >
											<?php if($campaign_details['status'] != "sent"){ ?>
											...
											<div class="d-block">
												...
												?>
													<span class="text-muted"><?php echo date('M d, o h:i:s A',strtotime($campaign_report['send_time'])); ?></span>
												<?php }else{ ?>
													<span class="text-muted"><?php echo date('M d, o h:i:s A',strtotime($campaign_details['create_time'])); ?></span>
												<?php	
													} 
												?>
											</div>
										...
										<div class="col-sm-6 col-md-4">
											<div class="d-flex justify-content-between">
												 <div class="text-right">
													 <span class="h4 text-blue d-block mb-1 font-weight-bold"><?php echo round(($campaign_report['opens']['unique_opens']/$campaign_report['emails_sent'])*100,2); ?>%</span>
													 <span class="text-muted text-uppercase d-block">OPEN RATE</span>
												</div>
												<div class="text-right">
													 <span class="h4 text-blue d-block mb-1 font-weight-bold"><?php echo round(($campaign_report['clicks']['unique_clicks']/$campaign_report['emails_sent'])*100,2);?>%</span>
												...
												<div class="text-right">
													 <div class="dropdown">
													  ...
													  <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
														...
														<?php } ?>
														<?php if($campaign_details['status'] == "save" || $campaign_details['status'] == "paused"){ ?>
															<a class="dropdown-item" href="edit-campaign.php?edit-campaign-id=<?php echo $campaign_details['id']; ?>">Edit</a>
														<?php } ?>
														<?php if($campaign_details['status'] == "schedule"){ ?>
															<a class="dropdown-item" href="" id="pause_<?php echo $campaign_details['id']?>" data-toggle="modal" data-target="#pauseCampaignModal" data-backdrop="static" data-keyboard="false" onclick="pauseCampaign(this.id)">Pause</a>
														<?php } ?>
													  </div>
													</div>
												</div>
	...
	function getCampaignInfo(linkElement){
		...
		$.ajax({
			type: "POST",
			url: "<?php  echo getFullUrl();  ?>/promote/campaign_ajax.php",
