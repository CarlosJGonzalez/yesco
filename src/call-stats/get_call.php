...
$call = $db->where("callid",$callId)->getOne("advtrack.calls");

if($statusFlag){
  if(!empty($call['recordurl']))
...
<small class="d-block mb-2">
  If you are having trouble listening to the call, <a href="<?php echo $callurl?>" class="text-blue" download>try downloading it here</a>.
</small>
<?php } ?>
<div class="form-group"><label><strong>Date/Time</strong></label><br>
<?php echo date("m/d/Y g:i:s A",strtotime($call['start']))?></div>

<div class="form-group"><label><strong>Caller #</strong></label><br>
<?php echo format_phone($call['caller'])?></p>

<div class="form-group"><label><strong>Caller ID</strong></label><br>
<?php echo format_phone($call['called'])?></p>
...
<textarea name="comment" class="form-control"><?php echo $call['comment']?></textarea>
</div>

<input name="callid" type="hidden" value="<?php echo $callId; ?>" />
<?php if($_POST['url']){ ?>
	<input name="header" type="hidden" value="<?php echo $_POST['url']?>" />
<?php }?>
