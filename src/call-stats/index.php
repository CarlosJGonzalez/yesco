      ...
      <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 p-0">
			...
			<div class="px-4 py-3">	
				...
				<?php
				$rct = 1;
				$sql= "select avg(time_to_sec(timediff(end,start))) as timediff,sum(if(duration>29,1,0)) as b,count(*) as c,sum(if(duplicate=1,1,0)) as d from advtrack.calls where client = '".$_SESSION['client']."-".$_SESSION['storeid']."' and start between '".$from." 00:00:00' and '".$to." 23:59:59'".$strcampid." and duration >= 30";
				...
				$sqlw = "select disposition,count(*) as c from advtrack.calls where client='".$_SESSION['client']."-".$_SESSION['storeid']."' and start between '".$from." 00:00:00' and '".$to." 23:59:59' ".$strcampid." and duration >=30  group by disposition";				
				...
				<div class="table-responsive">
					<table class="table" id="callsTable">
						...
						<tbody>
							...
							$sql="(select SQL_CALC_FOUND_ROWS duplicate,comment,client,disposition,campid,start,caller,called,cid,vendorid,recorded,ifnull(keywords,'') as keywords,rating,flagged,callid,recordurl,tmlogin,timediff(end,start) as d,(select concat(city,', ',state,' (',zipcode_postalcode,')') as loc from rates.areacodeworld where npa=left(a.caller,3) and nxx=mid(a.caller,4,3) order by zipcode_count desc limit 1) as loc, status from advtrack.calls a where client='".$_SESSION['client']."-".$_SESSION['storeid']."' ".$strcampid." and start between '".$from." 00:00:00' and '".$to." 23:59:59' and length(caller)=10 and CASE WHEN campid=20 THEN timediff(end,start)>'00:00:30' ELSE timediff(end,start)>'00:00:14' END) union all (select '0' as duplicate,comment,client,disposition,campid,start,caller,called,cid,'' as vendorid,recorded,'' as keywords,rating,flagged,callid,'' as recordurl,null as tmlogin,timediff(end,start) as d,(select concat(city,', ',state,' (',zipcode_postalcode,')') as loc from rates.areacodeworld where npa=left(a.caller,3) and nxx=mid(a.caller,4,3) order by zipcode_count desc limit 1) as loc, null as status from vxml.click2callcalls a where client='".$_SESSION['client']."-".$_SESSION['storeid']."' ".$strcampid." and start between '".$from." 00:00:00' and '".$to." 23:59:59' and length(caller)=10 and CASE WHEN campid=20 THEN timediff(end,start)>'00:00:30' ELSE timediff(end,start)>'00:00:14' END) "; 
							 
							//echo $sql;
							$calls_stats = $db->rawQuery($sql);
							if ($db->count > 0)
								foreach($calls_stats as $data){  
									if(strtolower($data['status']) == 'abandoned'){
										continue;
									}
							?>
							...
							<tr>
								...
								<td><button type="button" class="btn btn-primary callBtn" data-status="<?php echo $data['status']?>" data-callid="<?=$data['callid']?>" data-vendorid="<?=$data['vendorid']?>">Listen</button></td>
							</tr>
							<? } ?>
						</tbody>
					</table>
	...
	<script>
		...
		$(document).on('click','.callBtn',function(){
			...
			$.ajax({
				url: "get_call.php", 
				type:"POST",
				data:{"callid":callid,"vendorid":vendorid,"status":status},
